...
const SmoothSpinner = () => (
  <span className="inline-flex items-center justify-center mr-1.5 animate-spin">
    <svg 
...
export function SettingsForm({ initialUser, transactions: initialTransactions, defaultTab }: SettingsFormProps) {
  ...
  const handleSendEmailOTP = async () => {
    ...
    setOtpLoading(true)
    try {
      const res = await sendOtpAction("EMAIL", profile.email, "PROFILE_UPDATE")
      if (res.success) {
